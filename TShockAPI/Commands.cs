...
namespace TShockAPI
{
	...
	public static class Commands
	{
		...
		public static void InitCommands()
		{
			...
			add(new Command(Permissions.canlogin, AttemptLogin, "登入", "登陆", "登录", "login")
			{
				AllowServer = false,
				DoLog = false,
			...
			add(new Command(Permissions.ban, Ban, "封", "封禁", "ban")
			{
				HelpText = "Manages player bans."
			});
			add(new Command(Permissions.broadcast, Broadcast, "公告", "说", "broadcast", "bc", "say")
			{
			...
			add(new Command(Permissions.manageitem, ItemBan, "封物品", "封禁物品", "物品封禁", "itemban")
			{
				HelpText = "Manages item bans."
			});
            add(new Command(Permissions.manageprojectile, ProjectileBan, "封弹幕", "封禁弹幕", "弹幕封禁", "projban")
            {
                HelpText = "Manages projectile bans."
            });
			add(new Command(Permissions.managetile, TileBan, "封方块", "封禁方块", "方块封禁", "tileban")
			{
			...
			add(new Command(Permissions.annoy, Annoy, "干扰", "骚扰", "annoy")
			{
			...
			add(new Command(Permissions.maintenance, Off, "关闭", "退出", "off", "exit")
			{
				HelpText = "Shuts down the server while saving."
			});
			add(new Command(Permissions.maintenance, OffNoSave, "不保存关服", "不保存退出", "退出不保存", "exit-nosave")
			{
			...
			add(new Command(Permissions.give, Give, "给", "give", "g")
			{
			...
			add(new Command(Permissions.clearangler, ClearAnglerQuests, "清除渔夫任务", "clearangler")
			{
			...
			add(new Command(Permissions.eclipse, Eclipse, "日蚀", "日食", "eclipse")
			{
			...
			add(new Command(Permissions.godmode, ToggleGodMode, "上帝", "上帝模式", "godmode")
			{
			...
			add(new Command(Permissions.kill, Kill, "秒杀", "杀玩家", "kill")
			{
			...
			add(new Command(Permissions.whisper, Reply, "回", "回复", "reply", "r")
			{
			...
			add(new Command(Permissions.slap, Slap, "耳光", "巴掌", "扇人", "slap")
			{
			...
			add(new Command(Permissions.warp, Warp, "跳转", "跃迁", "翘曲", "warp")
			{
				HelpText = "Teleports you to a warp point or manages warps."
			});
			add(new Command(Permissions.whisper, Whisper, "私聊", "密语", "whisper", "w", "tell")
			{
			...
			add(new Command(Motd, "公告栏", "motd")
			{
				HelpText = "Shows the message of the day."
			});
			add(new Command(ListConnectedPlayers, "在线", "玩家", "playing", "online", "who")
			{
		...
		public static bool HandleCommand(TSPlayer player, string text)
		{
			...
			foreach (Command cmd in cmds)
			{
				...
				{
					player.SendErrorMessage("此命令只能在游戏中使用.");
				}
				else
				{
					if (cmd.DoLog)
		...
		private static void AttemptLogin(CommandArgs args)
		{
			if (args.Player.LoginAttempts > TShock.Config.MaximumLoginAttempts && (TShock.Config.MaximumLoginAttempts != -1))
			{
				...
				TShock.Utils.Kick(args.Player, "输错密码过多");
				return;
			}
			...
			else if (args.Parameters.Count == 2 && TShock.Config.AllowLoginAnyUsername)
			{
				...
				{
					args.Player.SendErrorMessage("登陆错误.");
					return;
				}
		...
		private static void ManageUsers(CommandArgs args)
		{
			...
			if ((subcmd == "add" || subcmd == "添加") && args.Parameters.Count == 4)
			{
				var user = new User();
			...
			else if ((subcmd == "del" || subcmd == "删除") && args.Parameters.Count == 2)
			{
				var user = new User();
				user.Name = args.Parameters[1];
			...
			else if ((subcmd == "password" || subcmd == "密码") && args.Parameters.Count == 3)
			{
				var user = new User();
				user.Name = args.Parameters[1];
			...
			else if ((subcmd == "group" || subcmd == "组") && args.Parameters.Count == 3)
			{
				var user = new User();
				user.Name = args.Parameters[1];
			...
			else if (subcmd == "help" || subcmd == "帮助")
			{
				args.Player.SendInfoMessage("命令帮助:");
				args.Player.SendInfoMessage("{0}用户 添加 用户名 密码 用户组   -- 添加用户", Specifier);
				args.Player.SendInfoMessage("{0}用户 删除 用户名                  -- 删除用户", Specifier);
				args.Player.SendInfoMessage("{0}用户 密码 用户名 密码 -- 更改用户密码", Specifier);
				args.Player.SendInfoMessage("{0}用户 组 用户名 用户组       -- 更改用户组", Specifier);
			}
			else
			{
				args.Player.SendErrorMessage("格式错误. 输入'{0}用户 帮助' 查看帮助", Specifier);
			...
			switch (subcmd)
			{
				...
				case "添加":
					#region Add ban
					{
						...
						{
							args.Player.SendErrorMessage("格式错误. 格式: {0}封 添加 <用户名> [原因]", Specifier);
							return;
						}
							...
							if (user != null)
							{
								...
								else
								{
									...
									else
									{
										if (args.Silent)
										{
											args.Player.SendInfoMessage("你封禁了{0}, 原因是'{1}'.", user.Name, reason);
										}
										else
										{
											TSPlayer.All.SendInfoMessage("{0}封禁了{1}, 原因是'{2}'.", args.Player.Name, user.Name, reason);
										}
									}
								}
							}
				...
				case "添加ip":
					#region Add IP ban
					{
						if (args.Parameters.Count < 2)
						{
							args.Player.SendErrorMessage("格式错误. 格式: {0}封 添加ip <IP> [原因]", Specifier);
							return;
						}
				...
				case "临时":
					#region Add temp ban
					{
						...
						{
							args.Player.SendErrorMessage("格式错误. 格式: {0}封 临时 <用户名> <时间> [原因]", Specifier);
							return;
						}
						...
						else
						{
								...
								false, args.Player.Name, DateTime.UtcNow.AddSeconds(time).ToString("s")))
							{
								...
								if (args.Player.RealPlayer)
									if (args.Silent)
									{
										args.Player.SendSuccessMessage("你封禁了{0}, 原因是'{1}'", players[0].Name, reason);
									}
									else
									{
										TSPlayer.All.SendSuccessMessage("{0}封禁了{1}, 原因是'{2}'", args.Player.Name, players[0].Name, reason);
									}
								else
								{
									if (args.Silent) 
									...
									{
										TSPlayer.All.SendSuccessMessage("{0}被封禁了, 原因是'{1}'. '", players[0].Name, reason);
									}
								}
							}
							else
				...
				case "删除":
					#region Delete ban
					{
						if (args.Parameters.Count != 2)
						{
							args.Player.SendErrorMessage("格式错误. 格式: {0}封 解除 <用户名>", Specifier);
							return;
						}
				...
				case "删除ip":
					#region Delete IP ban
					{
						if (args.Parameters.Count != 2)
						{
							args.Player.SendErrorMessage("格式错误. 格式: {0}封 解除ip <IP>", Specifier);
							return;
						}
					...
					#region Help
					{
						...
						PaginationTools.SendPage(args.Player, pageNumber, lines,
							new PaginationTools.Settings
							{
								...
								FooterFormat = "输入 {0}封 帮助 {{0}} 查看子命令.".SFormat(Specifier)
							}
						);
					}
					#endregion
				...
				case "列表":
					#region List bans
					{
						...
						PaginationTools.SendPage(args.Player, pageNumber, PaginationTools.BuildLinesFromTerms(nameBans),
							...
							{
								HeaderFormat = "封禁 ({0}/{1}):",
								FooterFormat = "输入 {0}封 列表 {{0}} 查看更多.".SFormat(Specifier),
				...
				case "ip列表":
					#region List IP bans
					{
						...
						PaginationTools.SendPage(args.Player, pageNumber, PaginationTools.BuildLinesFromTerms(ipBans),
							new PaginationTools.Settings
							{
								...
								FooterFormat = "输入 {0}封 ip列表 {{0}} 查看更多.".SFormat(Specifier),
					...
					return;
				default:
					args.Player.SendErrorMessage("格式错误. 输入{0}封 帮助 查看更多信息.", Specifier);
					return;
			}
		}
		...
		private static void OverrideSSC(CommandArgs args)
		{
			...
			{
				args.Player.SendErrorMessage("玩家 \"{0}\" 已经登陆.", matchedPlayer.Name);
				return;
			}
			if (!matchedPlayer.LoginFailsBySsi)
			{
				args.Player.SendErrorMessage("玩家 \"{0}\" 需要先执行一次/登入.", matchedPlayer.Name);
				return;
			}
			if (matchedPlayer.IgnoreActionsForClearingTrashCan)
			{
				args.Player.SendErrorMessage("玩家 \"{0}\"需要重新连接.", matchedPlayer.Name);
				return;
			}
		...
		private static void Restart(CommandArgs args)
		{
			if (TShock.NoRestart)
			{
				args.Player.SendErrorMessage("此命令被禁用.");
				return;
			}
		...
		private static void Invade(CommandArgs args)
		{
			if (Main.invasionSize <= 0)
			{
				if (args.Parameters.Count < 1)
				{
                    ...
                    args.Player.SendErrorMessage("入侵名如下 海盗 哥布林 雪人 南瓜月 霜月 火星", Specifier);
                    return;
				}
		...
		private static void SpawnBoss(CommandArgs args)
		{
			...
			switch (args.Parameters[0].ToLower())
			{
                ...
                case "肉山":
					if (Main.wof >= 0)
					{
						args.Player.SendErrorMessage("不能同时召唤两个血肉之墙.");
						return;
					}
                ...
                case "月总":
					...
					return;
				default:
					args.Player.SendErrorMessage("Boss名称错误.");
					return;
			}
		}
		...
		private static void SpawnMob(CommandArgs args)
		{
			if (args.Parameters.Count < 1 || args.Parameters.Count > 2)
			{
				args.Player.SendErrorMessage("格式错误. 格式: {0}刷怪 <怪物类型> [数量]", Specifier);
				return;
			}
			if (args.Parameters[0].Length == 0)
			{
		...
		private static void TP(CommandArgs args)
		{
			if (args.Parameters.Count != 1 && args.Parameters.Count != 2)
			{
				if (args.Player.Group.HasPermission(Permissions.tpothers))
					args.Player.SendErrorMessage("格式错误. 格式: {0}传送 <玩家名> [玩家2名]", Specifier);
				else
		...
		private static void TPNpc(CommandArgs args)
		{
            ...
            foreach (var npc in Main.npc.Where(npc => npc.active))
            {
                if (string.Equals(npc.name, npcStr, StringComparison.CurrentCultureIgnoreCase) || 
            	    string.Equals(Touhou.Terraria语言包.怪物名(npc.type), npcStr, StringComparison.CurrentCultureIgnoreCase))
                {
                    matches = new List<NPC> { npc };
                    break;
                }
                if (npc.name.ToLower().StartsWith(npcStr.ToLower()) || Touhou.Terraria语言包.怪物名(npc.type).ToLower().StartsWith(npcStr.ToLower()))
                    matches.Add(npc);
            }
		...
		private static void TPPos(CommandArgs args)
		{
			if (args.Parameters.Count != 2)
			{
				args.Player.SendErrorMessage("格式错误. 格式: {0}去坐标 <x> <y>", Specifier);
				return;
			}
		...
		private static void Warp(CommandArgs args)
		{
            ...
            if (args.Parameters.Count < 1)
            {
                ...
                {
                    args.Player.SendInfoMessage("格式错误. 格式: {0}跳转 [子命令] [参数]", Specifier);
                    args.Player.SendInfoMessage("命令: 添加, 删除, 隐藏, 列表, 传送, [跳转点名]");
                    args.Player.SendInfoMessage("参数: 添加 [跳转点名], 删除 [跳转点名], 列表 [页码]");
                    args.Player.SendInfoMessage("参数: 传送 [玩家名] [跳转点名], 隐藏 [跳转点名] [开/关]");
                    args.Player.SendInfoMessage("例如, {0}跳转 添加 跳转点1, {0}跳转 隐藏 跳转点1 开, {0}跳转 跳转点1", Specifier);
                    return;
                }
                else
                {
                    args.Player.SendErrorMessage("格式错误. 格式: {0}跳转 [跳转点名] 或 {0}跳转 列表 <页码>", Specifier);
                    return;
                }
            }

			if (args.Parameters[0].Equals("list") || args.Parameters[0].Equals("列表"))
            {
                #region List warps
				int pageNumber;
				if (!PaginationTools.TryParsePageNumber(args.Parameters, 1, args.Player, out pageNumber))
				...
				PaginationTools.SendPage(args.Player, pageNumber, PaginationTools.BuildLinesFromTerms(warpNames),
					new PaginationTools.Settings
					{
						HeaderFormat = "跳转点 ({0}/{1}):",
						FooterFormat = "输入 {0}跳转 列表 {{0}} 查看更多.".SFormat(Specifier),
						NothingToDisplayString = "目前没有跳转点."
					});
                #endregion
            }
			else if ((args.Parameters[0].ToLower() == "add" || args.Parameters[0].ToLower() == "添加") && hasManageWarpPermission)
            {
                #region Add warp
                if (args.Parameters.Count == 2)
                {
                    string warpName = args.Parameters[1];
                    if (warpName == "list" || warpName == "hide" || warpName == "del" || warpName == "add" || warpName == "send" || warpName == "列表" || warpName == "隐藏" || warpName == "删除" || warpName == "添加" || warpName == "传送")
                    {
                        args.Player.SendErrorMessage("名称保留, 请换用另一个. .");
                    }
                    else if (TShock.Warps.Add(args.Player.TileX, args.Player.TileY, warpName))
                    {
                        args.Player.SendSuccessMessage("新建跳转点: " + warpName);
                    }
                    else
                    {
                        args.Player.SendErrorMessage("跳转点" + warpName + " 已经存在.");
                    }
                }
                else
                    args.Player.SendErrorMessage("格式错误. 格式: {0}跳转 添加 [跳转点名]", Specifier);
                #endregion
            }
			else if ((args.Parameters[0].ToLower() == "del" || args.Parameters[0].ToLower() == "删除") && hasManageWarpPermission)
            {
                #region Del warp
                if (args.Parameters.Count == 2)
                {
					...
					{
						args.Player.SendSuccessMessage("删除跳转点: " + warpName);
					}
					else
						args.Player.SendErrorMessage("找不到该点.");
                }
                else
                    args.Player.SendErrorMessage("格式错误. 格式: {0}跳转 删除 [跳转点名]", Specifier);
                #endregion
            }
			else if ((args.Parameters[0].ToLower() == "hide" || args.Parameters[0].ToLower() == "隐藏") && hasManageWarpPermission)
            {
                #region Hide warp
                if (args.Parameters.Count == 3)
                {
                    string warpName = args.Parameters[1];
                    bool state = false;
                    if (Boolean.TryParse(args.Parameters[2], out state) || (args.Parameters[2] == "开" || args.Parameters[2] == "关"))
                    {
                    	if (args.Parameters[2] == "开") state = true;
                    	if (args.Parameters[2] == "关") state = false;
                        if (TShock.Warps.Hide(args.Parameters[1], state))
                        {
                            if (state)
                                args.Player.SendSuccessMessage("跳转点 " + warpName + " 设为私人.");
                            else
                                args.Player.SendSuccessMessage("跳转点 " + warpName + " 设为公开.");
                        }
                        else
                            args.Player.SendErrorMessage("找不到该点.");
                    }
                    else
                        args.Player.SendErrorMessage("格式错误. 格式: {0}跳转 隐藏 [跳转点名] <开/关>", Specifier);
                }
                else
                    args.Player.SendErrorMessage("格式错误. 格式: {0}跳转 隐藏 [跳转点名] <开/关>", Specifier);
                #endregion
            }
			else if ((args.Parameters[0].ToLower() == "send" || args.Parameters[0].ToLower() == "传送") && args.Player.Group.HasPermission(Permissions.tpothers))
            {
                #region Warp send
                if (args.Parameters.Count < 3)
                {
                    args.Player.SendErrorMessage("格式错误. 格式: {0}跳转 传送 [玩家名] [跳转点名]", Specifier);
                    return;
                }
            ...
            else
            {
                ...
                if (warp != null)
                {
					if (args.Player.Teleport(warp.Position.X * 16, warp.Position.Y * 16))
                        args.Player.SendSuccessMessage("跳转到 " + warpName + ".");
                }
                else
                {
                    args.Player.SendErrorMessage("找不到该点.");
			...
			switch (subCmd)
			{
				...
				case "添加":
					#region Add group
					{
						if (args.Parameters.Count < 2)
						{
							args.Player.SendErrorMessage("格式错误. 格式: {0}组 添加 <组名> [权限]", Specifier);
							return;
						}
						...
						{
							args.Player.SendErrorMessage("该组已经存在.");
						}
				...
				case "添加权限":
					#region Add permissions
					{
						if (args.Parameters.Count < 3)
						{
							args.Player.SendErrorMessage("格式错误. 格式: {0}组 添加权限 <组名> <权限>", Specifier);
							return;
						}
					...
					#region Help
					{
						...
						PaginationTools.SendPage(args.Player, pageNumber, lines,
							new PaginationTools.Settings
							{
								...
								FooterFormat = "输入 {0}组 帮助 {{0}} 查看子命令.".SFormat(Specifier)
							}
						);
					}
					#endregion
				...
				case "父组":
					#region Parent
					{
						if (args.Parameters.Count < 2)
						{
							args.Player.SendErrorMessage("格式错误. 格式: {0}组 父组 <组名> [父组名]", Specifier);
							return;
						}
				...
				case "后缀":
					#region Suffix
					{
						if (args.Parameters.Count < 2)
						{
							args.Player.SendErrorMessage("格式错误. 格式: {0}组 后缀 <组名> [后缀]", Specifier);
							return;
						}
				...
				case "前缀":
					#region Prefix
					{
						if (args.Parameters.Count < 2)
						{
							args.Player.SendErrorMessage("格式错误. 格式: {0}组 前缀 <组名> [前缀]", Specifier);
							return;
						}
				...
				case "颜色":
					#region Color
					{
						...
						{
							args.Player.SendErrorMessage("格式错误. 格式: {0}组 颜色 <组名> [颜色(000,000,000)]", Specifier);
							return;
						}
						...
						if (args.Parameters.Count == 3)
						{
							...
							{
								args.Player.SendErrorMessage("颜色格式错误. 标准：\"红,绿,蓝\"");
							}
						}
						else
						{
				...
				case "删除":
					#region Delete group
					{
						if (args.Parameters.Count != 2)
						{
							args.Player.SendErrorMessage("格式错误. 格式: {0}组 删除 <组名>", Specifier);
							return;
						}
				...
				case "删除权限":
					#region Delete permissions
					{
						if (args.Parameters.Count < 3)
						{
							args.Player.SendErrorMessage("格式错误. 格式: {0}组 删除权限 <组名> <权限>", Specifier);
							return;
						}
				...
				case "列表":
					#region List groups
					{
						...
						PaginationTools.SendPage(args.Player, pageNumber, PaginationTools.BuildLinesFromTerms(groupNames),
							...
							{
								HeaderFormat = "组 ({0}/{1}):",
								FooterFormat = "输入 {0}组 列表 {{0}} 查看更多.".SFormat(Specifier)
							});
					}
					#endregion
					return;
				...
				case "权限列表":
					#region List permissions
					{
						if (args.Parameters.Count == 1)
						{
							args.Player.SendErrorMessage("格式错误. 格式: {0}组 权限列表 <组名> [页码]", Specifier);
							return;
						}
						int pageNumber;
						...
						PaginationTools.SendPage(args.Player, pageNumber, PaginationTools.BuildLinesFromTerms(permissions),
							new PaginationTools.Settings
							{
								...
								FooterFormat = "输入 {0}组 权限列表 {1} {{0}} 查看更多.".SFormat(Specifier, grp.Name),
			...
			switch (subCmd)
			{
				...
				case "添加":
					#region Add item
					{
						if (args.Parameters.Count != 2)
						{
							args.Player.SendErrorMessage("格式错误. 格式: {0}封物品 添加 <物品名>", Specifier);
							return;
						}
						...
						{
							TShock.Utils.SendMultipleMatchError(args.Player, items.Select(i => Touhou.Terraria语言包.物品名(i.type)));
						}
						else
						{
							TShock.Itembans.AddNewBan(items[0].name);
							args.Player.SendSuccessMessage("封禁 " + Touhou.Terraria语言包.物品名(items[0].type) + ".");
						}
					}
					#endregion
					return;
				...
				case "允许":
					#region Allow group to item
					{
						if (args.Parameters.Count != 3)
						{
							args.Player.SendErrorMessage("格式错误. 格式: {0}封物品 允许 <物品名> <组名>", Specifier);
							return;
						}
						...
						else
						{
							...
							if (ban == null)
							{
								args.Player.SendErrorMessage("{0} 没有被封禁.", Touhou.Terraria语言包.物品名(items[0].type));
								return;
							}
							if (!ban.AllowedGroups.Contains(args.Parameters[2]))
							{
								TShock.Itembans.AllowGroup(items[0].name, args.Parameters[2]);
								args.Player.SendSuccessMessage("{0} 被允许使用物品 {1}.", args.Parameters[2], Touhou.Terraria语言包.物品名(items[0].type));
							}
							else
							{
								args.Player.SendWarningMessage("{0} 已被允许使用 {1}.", args.Parameters[2], Touhou.Terraria语言包.物品名(items[0].type));
							}
						}
					}
					#endregion
				...
				case "解除":
					#region Delete item
					{
						if (args.Parameters.Count != 2)
						{
							args.Player.SendErrorMessage("格式错误. 格式: {0}封物品 解除 <物品名>", Specifier);
							return;
						}
						...
						else
						{
							...
							args.Player.SendSuccessMessage("解除封禁 " + Touhou.Terraria语言包.物品名(items[0].type) + ".");
						}
					}
					#endregion
					return;
				...
				case "禁止":
					#region Disllow group from item
					{
						if (args.Parameters.Count != 3)
						{
							args.Player.SendErrorMessage("格式错误. 格式: {0}封物品 禁止 <物品名> <组名>", Specifier);
							return;
						}
						...
						else
						{
							...
							if (ban.AllowedGroups.Contains(args.Parameters[2]))
							{
								...
								args.Player.SendSuccessMessage("{0} 被禁止使用物品 {1}.", args.Parameters[2], Touhou.Terraria语言包.物品名(items[0].type));
							}
							else
							{
								args.Player.SendWarningMessage("{0} 已被禁止使用 {1}.", args.Parameters[2], Touhou.Terraria语言包.物品名(items[0].type));
							}
						}
					}
					#endregion
				...
				case "帮助":
					#region Help
					{
						...
						{
							"添加 <物品名> - 封禁一种物品.",
							"允许 <物品名> <组> - 允许一个组使用特定物品.",
							"解除 <物品名> - 解除物品封禁.",
							"禁止 <物品名> <组> - 禁止一个组使用特定物品.",
							"列表 [页码] - 列出所有物品封禁."
                        };
						...
						PaginationTools.SendPage(args.Player, pageNumber, lines,
							new PaginationTools.Settings
							{
								...
								FooterFormat = "输入 {0}封物品 帮助 {{0}} 查看子命令.".SFormat(Specifier)
							}
						);
					}
					#endregion
				...
				case "列表":
					#region List items
					{
						...
						IEnumerable<string> itemNames = from itemBan in TShock.Itembans.ItemBans
														select Touhou.Terraria语言包.物品名(TShock.Utils.GetItemByName(itemBan.Name)[0].type) + "(" + itemBan.Name + ")";
						PaginationTools.SendPage(args.Player, pageNumber, PaginationTools.BuildLinesFromTerms(itemNames),
							new PaginationTools.Settings
							{
								HeaderFormat = "物品封禁 ({0}/{1}):",
								FooterFormat = "输入 {0}封物品 列表 {{0}} 查看更多.".SFormat(Specifier),
			...
			switch (subCmd)
			{
				...
				case "添加":
					#region Add projectile
					{
						if (args.Parameters.Count != 2)
						{
							args.Player.SendErrorMessage("格式错误. 格式: {0}封弹幕 添加 <弹幕ID>", Specifier);
							return;
						}
						short id;
				...
				case "允许":
					#region Allow group to projectile
					{
						if (args.Parameters.Count != 3)
						{
							args.Player.SendErrorMessage("格式错误. 格式: {0}封弹幕 允许 <弹幕ID> <组>", Specifier);
							return;
						}
				...
				case "解除":
					#region Delete projectile
					{
						if (args.Parameters.Count != 2)
						{
							args.Player.SendErrorMessage("格式错误. 格式: {0}封弹幕 解除 <弹幕ID>", Specifier);
							return;
						}
				...
				case "禁止":
					#region Disallow group from projectile
					{
						if (args.Parameters.Count != 3)
						{
							args.Player.SendErrorMessage("格式错误. 格式: {0}封弹幕 禁止 <弹幕ID> <组名>", Specifier);
							return;
						}
				...
				case "帮助":
					#region Help
					{
						...
						{
							"添加 <弹幕ID> - 封禁一种弹幕.",
							"允许 <弹幕ID> <组> - 允许一个组使用特定弹幕.",
							"解除 <弹幕ID> - 解除弹幕封禁.",
							"禁止 <弹幕ID> <组> - 禁止一个组使用特定弹幕.",
							"列表 [页码] - 列出所有弹幕封禁."
                        };
						...
						PaginationTools.SendPage(args.Player, pageNumber, lines,
							new PaginationTools.Settings
							{
								...
								FooterFormat = "输入 {0}封弹幕 帮助 {{0}} 查看子命令.".SFormat(Specifier)
							}
						);
					}
					#endregion
					...
					#region List projectiles
					{
						...
						PaginationTools.SendPage(args.Player, pageNumber, PaginationTools.BuildLinesFromTerms(projectileIds),
							new PaginationTools.Settings
							{
								...
								FooterFormat = "输入 {0}封弹幕 列表 {{0}} 查看更多.".SFormat(Specifier),
			...
			switch (subCmd)
			{
				...
				case "添加":
					#region Add tile
					{
						if (args.Parameters.Count != 2)
						{
							args.Player.SendErrorMessage("格式错误. 格式: {0}封方块 添加 <方块ID>", Specifier);
							return;
						}
						short id;
				...
				case "允许":
					#region Allow group to place tile
					{
						if (args.Parameters.Count != 3)
						{
							args.Player.SendErrorMessage("格式错误. 格式: {0}封方块 允许 <方块ID> <组>", Specifier);
							return;
						}
				...
				case "解除":
					#region Delete tile ban
					{
						if (args.Parameters.Count != 2)
						{
							args.Player.SendErrorMessage("格式错误. 格式: {0}封方块 解除 <方块ID>", Specifier);
							return;
						}
				...
				case "禁止":
					#region Disallow group from placing tile
					{
						if (args.Parameters.Count != 3)
						{
							args.Player.SendErrorMessage("格式错误. 格式: {0}封方块 禁止 <方块ID> <组名>", Specifier);
							return;
						}
					...
					#region Help
					{
						...
						PaginationTools.SendPage(args.Player, pageNumber, lines,
							new PaginationTools.Settings
							{
								...
								FooterFormat = "输入 {0}封方块 帮助 {{0}} 查看子命令.".SFormat(Specifier)
							}
						);
					}
					#endregion
					...
					#region List tile bans
					{
						...
						PaginationTools.SendPage(args.Player, pageNumber, PaginationTools.BuildLinesFromTerms(tileIds),
							new PaginationTools.Settings
							{
								...
								FooterFormat = "输入 {0}封方块 列表 {{0}} 查看更多.".SFormat(Specifier),
		...
		private static void MaxSpawns(CommandArgs args)
		{
			...
			if (String.Equals(args.Parameters[0], "default", StringComparison.CurrentCultureIgnoreCase) || String.Equals(args.Parameters[0], "默认", StringComparison.CurrentCultureIgnoreCase))
			{
		...
		private static void Rain(CommandArgs args)
		{
			if (args.Parameters.Count < 1 || args.Parameters.Count > 2)
			{
				args.Player.SendErrorMessage("格式错误. 格式: {0}下雨 [史莱姆] <开始/结束>", Specifier);
				return;
			}

			int switchIndex = 0;
			if (args.Parameters.Count == 2 && (args.Parameters[0].ToLowerInvariant() == "slime" || args.Parameters[0].ToLowerInvariant() == "史莱姆"))
			{
				switchIndex = 1;
			}
			...
			switch (args.Parameters[switchIndex].ToLower())
			{
				...
				case "开始":
					if (switchIndex == 1)
					{
						...
						TSPlayer.All.SendData(PacketTypes.WorldInfo);
						TSPlayer.All.SendInfoMessage("{0}招来了史莱姆雨.", args.Player.Name);
					}
					else
					{
						Main.StartRain();
						TSPlayer.All.SendData(PacketTypes.WorldInfo);
						TSPlayer.All.SendInfoMessage("{0}招来了雨.", args.Player.Name);
					}
					break;
				case "stop":
				case "停止":
				...
				case "终止":
					if (switchIndex == 1)
					{
						...
						TSPlayer.All.SendData(PacketTypes.WorldInfo);
						TSPlayer.All.SendInfoMessage("{0}终止了史莱姆雨.", args.Player.Name);
					}
					else
					{
						Main.StopRain();
						TSPlayer.All.SendData(PacketTypes.WorldInfo);
						TSPlayer.All.SendInfoMessage("{0}终止了雨.", args.Player.Name);
					}
					break;
				default:
		...
		private static void Slap(CommandArgs args)
		{
			if (args.Parameters.Count < 1 || args.Parameters.Count > 2)
			{
				args.Player.SendErrorMessage("格式错误. 格式: {0}扇人 <玩家名> [伤害]", Specifier);
				return;
			}
			if (args.Parameters[0].Length == 0)
			{
		...
		private static void Help(CommandArgs args)
		{
			...
			if (args.Parameters.Count == 0 || int.TryParse(args.Parameters[0], out pageNumber))
			{
				...
				IEnumerable<string> cmdNames = from cmd in ChatCommands
											   where cmd.CanRun(args.Player) && (cmd.Name != "auth" || TShock.AuthToken != 0) && (cmd.ShowInHelp)
		...
		{
			args.Player.SendInfoMessage("TShock: {0} ({1}) 汉化版 Beta {2}", TShock.VersionNum, TShock.VersionCodename, TShock.CNVersion);
		}
		...
		private static void ListConnectedPlayers(CommandArgs args)
		{
			...
			if (invalidUsage)
			{
				args.Player.SendErrorMessage("格式错误. 格式: {0}在线 [-i] [页码]", Specifier);
				return;
			}
		...
		private static void AuthToken(CommandArgs args)
		{
			...
			{
				args.Player.SendInfoMessage("请关闭认证系统. 如果需要帮助, 请联系我们的论坛http://tshockcn.com/");
		...
		private static void AuthVerify(CommandArgs args)
		{
			...
			args.Player.SendSuccessMessage("TShock中文论坛：https://tshockcn.com/");
		...
		private static void Whisper(CommandArgs args)
		{
			if (args.Parameters.Count < 2)
			{
				args.Player.SendErrorMessage("格式错误. 格式: {0}私聊 <玩家名> <文本>", Specifier);
				return;
			}
		...
		private static void Annoy(CommandArgs args)
		{
			if (args.Parameters.Count != 2)
			{
				args.Player.SendErrorMessage("格式错误. 格式: {0}骚扰 <玩家名> <秒数>", Specifier);
				return;
			}
			int annoy = 5;
		...
		private static void FireWork(CommandArgs args)
		{
			if (args.Parameters.Count < 1)
			{
				args.Player.SendErrorMessage("格式错误. 格式: {0}烟花 <玩家名> [红|绿|蓝|黄]", Specifier);
				return;
			}
			var players = TShock.Utils.FindPlayer(args.Parameters[0]);
			if (players.Count == 0)
			...
			else
			{
				...
				if (args.Parameters.Count > 1)
				{
					if (args.Parameters[1].ToLower() == "green" || args.Parameters[1].ToLower() == "绿")
						type = 168;
					else if (args.Parameters[1].ToLower() == "blue" || args.Parameters[1].ToLower() == "蓝")
						type = 169;
					else if (args.Parameters[1].ToLower() == "yellow" || args.Parameters[1].ToLower() == "黄")
						type = 170;
				}
				var ply = players[0];
				...
				if (item.type >= 1 && item.type < Main.maxItemTypes)
				{
					...
					else
					{
						...
						if (plr.InventorySlotAvailable || (item.type > 70 && item.type < 75) || item.ammo > 0 || item.type == 58 || item.type == 184)
						{
							...
							else if (plr.GiveItemCheck(item.type, Touhou.Terraria语言包.物品名(item.type), item.width, item.height, itemAmount, prefix))
							{
								args.Player.SendSuccessMessage(string.Format("给了{0} {1}个{2}.", plr.Name, itemAmount, item.name));
								plr.SendSuccessMessage(string.Format("{0} 给了你 {1}个{2}.", args.Player.Name, itemAmount, item.name));
							}
